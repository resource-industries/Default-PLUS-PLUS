...
local fuelCounter = 1
local abs, floor, format, sqrt, clamp = math.abs, math.floor, string.format, math.sqrt, utils.clamp
...
function WidgetsPlusPlusCustom.SVG_Update(self)
    ...
    local throttleText = format("%.0f", ThrottlePos).."kph"
    if MasterMode == "TRAVEL" then throttleText = format("%.0f", ThrottlePos*100).."/100" end
    ...
    local SVGspeedo = [[
        ...
        <text x="170" y="500" font-size="15" text-anchor="end" font-family="]]..widget_font..[[" alignment-baseline="middle" stroke-width="0" fill="]]..WTC..[[">]]..format("%.0f", abs(zSpeedKPH))*utils.sign(zSpeedKPH).."kph"..[[</text>
    ...
    local AHT = utils.sign(Ax0) * abs(Ax0) / 90 * 400
