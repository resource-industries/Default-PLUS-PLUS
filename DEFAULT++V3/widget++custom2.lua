    ...
    if params.Travel_Planner.Destination.value ~= nil and #params.Travel_Planner.Destination.value == 3 then
        local bf = function() return function()
                            ...
                            elseif mouseWheel > 0 then
                                self.stopDist = clamp(self.stopDist+0.05,0.05,25)
                            elseif mouseWheel < 0 then
                                self.stopDist = clamp(self.stopDist-0.05,0.05,25)
                            end
                            windowsShow()
        ...
        self.buttons = {
            {btText, bf(), {name = "traveler++ start", class = nil, width = 250, height = 25, posX = 0, posY = 120}},   -- class = "separator"   (for invisible background button)
            }
    else
        self.buttons = {
